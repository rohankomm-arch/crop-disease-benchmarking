...
class LogisticRegressionModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.linear = nn.Linear(3 * 64 * 64, num_classes)

    def forward(self, x):
        x = x.view(x.size(0), -1)
        return self.linear(x)

model = LogisticRegressionModel(num_classes=num_classes).to(device)
...
for fold, (train_indices, val_indices) in enumerate(skf.split(all_indices, all_labels)):
    ...
    for epoch in range(num_epochs):
        ...
        for images, labels in train_loader:
            images, labels = images.to(device), labels.to(device) # FIXED: Move to current active device
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            best_epoch = epoch + 1
            torch.save(model.state_dict(), f'./best_lr_fold{fold+1}.pth')
    ...
    model.load_state_dict(torch.load(f'./best_lr_fold{fold+1}.pth'))
...
plt.xlabel('Fold')
plt.ylabel('F1 Score')
plt.title('F1 Scores Across Folds - Logistic Regression')  # change title per model
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('Confusion Matrix - Logistic Regression (All Folds Aggregated)')
...
axes[0].set_title('F1 Scores Across Folds - Logistic Regression')
...
axes[1].set_title('Precision and Recall Across Folds - Logistic Regression')
...
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.title('Training and Validation Accuracy - Logistic Regression (All Folds)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Training and Validation Loss - Logistic Regression (All Folds)')
plt.legend()
plt.grid(True)
plt.show()
...
plt.title('Best Epoch per Fold - Logistic Regression')
plt.xticks(folds)
plt.legend()
